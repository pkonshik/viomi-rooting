...
function main() {
  [ -d "$HOME/.ssh" ] || {
    echo "$HOME/.ssh does not exist, trying to create it."
...
EOF
  ...
  if ssh -o ConnectTimeout=5 -q vacuum exit; then
...
EOF
    ...
    read -p 'Press ENTER once ADB setup is completed.' -r
  fi
...
function install_valetudo() {
  wget "https://github.com/Hypfer/Valetudo/releases/download/2022.09.0/valetudo-armv7" -O valetudo
  chmod +x valetudo
